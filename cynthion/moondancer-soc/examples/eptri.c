 ...
 */
 void send_packet(uint8_t endpoint, const void *data, uint16_t length)
 {
    ...
    usb0_ep_in_reset_write(1);

    // Send data until we run out of bytes.
    while (length) {
        usb0_ep_in_data_write(*buffer);
    ...
    usb0_ep_in_epno_write(endpoint);
...
void prime_receive(uint8_t endpoint)
{
    ...
    usb0_ep_out_epno_write(endpoint);
    usb0_ep_out_enable_write(1);
}
...
{
    usb0_ep_in_stall_write(1);
    usb0_ep_out_stall_write(1);
}
...
void handle_setup_request(void)
{
    ...
    switch (last_setup_packet.bRequest) {
        ...
        case REQUEST_SET_ADDRESS:
            uart_puts("handle_set_address\n");
            handle_set_address();
            break;

        case REQUEST_GET_DESCRIPTOR:
            uart_puts("handle_get_descriptor\n");
            handle_get_descriptor();
            break;

        case REQUEST_SET_CONFIGURATION:
            uart_puts("handle_set_configuration\n");
            handle_set_configuration(last_setup_packet.wValue);
            break;

        default:
            uart_puts("unhandled_request\n");
            unhandled_request();
            break;
    }
}
