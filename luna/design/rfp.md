...
Investigation: Implement the firmware in a pure Rust `no_std` environment without the use of the `alloc` feature.
...
Peripheral interfaces are provided by two crates:

* `lunasoc-pac` - A peripheral register access API generated by `svd2rust`
* `lunasoc-hal` - A concrete implementation of the peripheral drivers conforming to device-independent `embedded-hal` traits.
...
* are there any constraints on the "Great Communications Protocol" implementation
  that will place limits on what we can do with Luna vs GreatFET?
